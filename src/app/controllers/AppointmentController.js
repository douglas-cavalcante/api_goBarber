  ...
  async store(req, res) {
    ...
    if (provider_id === req.userId) {
      return res.status(401).json({
        error: 'Scheduling is not allowed with yourself',
      });
    }
    ...
    if (!checkIsProvider) {
      return res.status(401).json({
        error: 'You can only create appointmnets with providers',
      });
    }

    /* validacao da data escolhida */
    ...
    /* validação da existência de agendamentos no mesmo horário */
